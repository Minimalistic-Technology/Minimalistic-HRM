...
const CheckInOutApp: React.FC = () => {
  ...
  // Save location to database - UPDATED VERSION WITH USER ID
  const saveLocationToDatabase = async (geoData: GeolocationData): Promise<Location | null> => {
    try {
      ...
      // Check if similar location already exists for this user to avoid duplicates
      const existingLocation = locations.find(loc => 
        loc.userId === userId &&
        loc.city === (geoData.city || "Current Location") && 
        loc.state === (geoData.state || "Unknown State") && 
      ...
      // Prepare location data for API - include userId
      const locationData = {
        ...
        country: geoData.country || "Unknown Country",
        ip: userIP,
        // Additional fields for future use (add to your backend schema if needed)
        coordinates: {
          latitude: geoData.latitude,
          longitude: geoData.longitude
        },
  ...
  // Handle getting current location - UPDATED VERSION
  const handleGetCurrentLocation = async () => {
    if (isCheckedIn) return; // Don't allow location change while checked in
    ...
    try {
      ...
      // Save location to database (this will now include userId)
  ...
  const fetchUserLocations = async (token: string, userIdParam: string): Promise<Location[]> => {
    try {
      ...
      // Handle different response formats
      let allLocations = Array.isArray(response.data) 
        ? response.data 
        : (response.data.locations || []);
      
      // Filter locations by userId (if backend doesn't do it automatically)
      const userLocations = allLocations.filter((loc: Location) => loc.userId === userIdParam);
      
      // Remove duplicate locations based on city, state, country combination for this user
      const uniqueLocations = userLocations.filter((location: { city: any; state: any; country: any; }, index: any, self: any[]) => 
        index === self.findIndex((l) => 
          l.city === location.city && 
          l.state === location.state && 
          l.country === location.country
  ...
  // Separate useEffect for fetching data when userId changes - UPDATED VERSION
  useEffect(() => {
    if (!userId) return; // Don't fetch if userId is not set yet
    ...
    const fetchData = async () => {
      try {
        ...
        // Fetch user-specific location data - UPDATED
        let fetchedLocations: Location[] = [];
        try {
          fetchedLocations = await fetchUserLocations(token, userId);
          setLocations(fetchedLocations);
          console.log(`Loaded ${fetchedLocations.length} locations for user ${userId}`);
          
          // Set location based on active session or existing current location
          if (activeSession) {
            // If there's an active session, try to find the matching location
            const sessionLocation = fetchedLocations.find(loc => 
              `${loc.city}, ${loc.state}, ${loc.country}` === activeSession.location
            );
            if (sessionLocation) {
              setLocation(`${sessionLocation.city}, ${sessionLocation.state}, ${sessionLocation.country}`);
            } else if (fetchedLocations.length > 0 && !location) {
              // Fallback to first available location if no current location
              setLocation(`${fetchedLocations[0].city}, ${fetchedLocations[0].state}, ${fetchedLocations[0].country}`);
            }
          } else if (fetchedLocations.length > 0 && !location && !useCurrentLocation) {
            // No active session and no current location set, use first available location
            setLocation(`${fetchedLocations[0].city}, ${fetchedLocations[0].state}, ${fetchedLocations[0].country}`);
          }
        } catch (locationError) {
          console.warn("Failed to fetch user locations, continuing without locations");
        }
